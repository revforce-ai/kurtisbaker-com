# AIR® Organizational-Document Catalog

The master list the responder draws from. This doubles as the **recommended
full set of documents to make available** — the items vendors (onboarding /
payment setup) and grant applications (due diligence) most commonly request
from a 501(c)(3).
...
## Tier 3 — Never auto-send (omit + redirect to a secure channel; flag for Kurt)
| # | Item | Why blocked |
|---|---|---|
| 18 | **Banking / ACH form / voided check / direct-deposit info** | Wire-fraud / BEC target; send only via a verified, out-of-band secure channel |
| 19 | **Anything containing an SSN, or non-public personnel/financial data** | Privacy / sensitivity |
| 20 | **Notarized or wet-signature originals** | Require manual handling |

---

## Request → document mapping (quick reference)
| Requester says… | Attach / include |
|---|---|
| "your 501c3 / determination letter / proof of tax-exempt status" | #1 (+ EIN inline) |
| "W-9" / "vendor onboarding" / "set you up as a payee" | #11 W-9 (+ #1) — restricted |
| "your most recent 990 / financials" | #3 (audited #15 if asked + available) |
| "proof of insurance / COI / certificate of insurance" | #12 — restricted; if stale, request fresh COI from broker |
| "charity registration number / registered to solicit" | #4 (+ reg # inline) |
| "sales tax exemption / ST-5" | #13 — restricted |
| "good standing / active status" | #14 — restricted |
...
- Replace **Form 990** when the new return is filed.
- Refresh the **COI** when the policy renews; never send an expired certificate.
- Re-verify the **charity registration** is current at its renewal date.
